...
def main():
    ...
    try:
        subprocess.run("node -v", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
        has_node = True
    except (subprocess.SubprocessError, FileNotFoundError):
        print("Node.js is not detected in PATH. Static server will fallback to serving pre-compiled files.")
        has_node = False
    ...
    try:
        ...
        subprocess.run(f"{sys.executable} -m uvicorn app.main:app --host 127.0.0.1 --port 8000", shell=True, cwd=root_dir)
